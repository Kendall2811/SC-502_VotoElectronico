...
class Candidato {
    ...
    public function registrar() {
        $query = "INSERT INTO " . $this->table_name . " SET id_eleccion=:id_eleccion, nombre=:nombre, partido=:partido";
    ...
    public function leerPorEleccion() {
        $query = "SELECT id_candidato as id, nombre, partido, id_eleccion as eleccion_id FROM " . $this->table_name . " WHERE id_eleccion = :id_eleccion";
        $stmt = $this->conn->prepare($query);
        $this->eleccion_id = htmlspecialchars(strip_tags($this->eleccion_id));
        $stmt->bindParam(":id_eleccion", $this->eleccion_id);
        $stmt->execute();
